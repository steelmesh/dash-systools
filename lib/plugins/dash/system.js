...
exports.connect = function(server, steelmesh, dash) {
    var navItems = [
    ...
    server.get('/system/config/reload', _makeReloadHandler(steelmesh.config, dash));
    server.get('/system/logs/download', _makeDownloadLogHandler(steelmesh.config, dash));
    ...
    return {
        loaders: {
            'system/config': function(req, page, callback) {
                callback({
                    config: JSON.stringify(steelmesh.config),
            ...
            'system/workers': _getWorkers(steelmesh.config, dash)
        },
...
exports.drop = function(server, steelmesh, dash) {
